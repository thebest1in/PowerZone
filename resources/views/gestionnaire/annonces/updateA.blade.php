...
<div class="intro-section" id="home" style="background-color: #eee ;">
      <div class="container">
        <div class="row align-items-center">
          ...
          <form action="/modifierAnnonces" method="post" style="margin:20px">
                @csrf
                @foreach($annonces as $i)
                <div class="form-group">  
                  <label>titre</label>
                  <input type="text" class="form-control" name="title" value="{{$i->title}}" style="margin:20px" required>
                </div>
                <div class="form-group">  
                  <label>date debut</label>
                  <input type="date" class="form-control" name="dateD" value="{{$i->dateDebut}}" style="margin:20px" required>
                </div>
                <div class="form-group">  
                  <label>date fin</label>
                  <input type="date" class="form-control" name="dateF" value="{{$i->dateFin}}" style="margin:20px" required>
                </div>
                <div class="form-group">contenu:
                  <textarea name="text" class="form-control" rows="8" style="margin:20px" required>{{$i->contenue}}</textarea>
                  <input type="hidden" class="form-control" name="id" value="{{$i->idAn}}" style="margin:20px">
                </div>
                <div class="form-group">  
